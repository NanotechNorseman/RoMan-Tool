...
<# 
 ...
 .Parameter MaxFileSize
  Minimum file size in bytes to include in the report.
  Default is 999,999 PB (1,125,898,780,942,717,157,376 Bytes).
  Example: 100MB => this will report on files that are 100MB or smaller only
 ...
 .Example
  Report-onFiles -TargetFolders C:\Sandbox 
  This example reports on all files under c:\sandbox folder

 .Example
  Report-onFiles "C:\Sandbox" , "\\Server5\Scripts\"
  This example reports on all files under C:\Sandbox & \\Server5\Scripts\ folders

 .Example
  Report-onFiles -TargetFolders "C:\Sandbox" -FileTypes txt,cs*
  This example reports on all files under C:\Sandbox with txt or cs* extensions

 .Example
  Report-onFiles -TargetFolders "C:\Sandbox" -FileTypes txt,cs* -Properties "Name","DirectoryName","Extension","Length","LastAccessTime"
 ...
 .Example
  Report-onFiles -TargetFolders C:\Sandbox,\\Server5\Scripts\ -FileTypes ps?,cs* -MinFileSize 1KB -SizeUnit KB
  This example reports on all files 
 ...
 .Notes
  ...
  15 July 2014 - V1.0
  20 July 2014 - V1.1 - updated Log function, cosmetic changes
  3 November 2014 - v1.2
...
#>
    ...
    Param(
        ...
        [Parameter(Mandatory=$false,
                   Position=3)]
            [Decimal]$MaxFileSize = 1125898780942717157376, 
        [Parameter(Mandatory=$false,
                   Position=4)]
        ...
        [Parameter(Mandatory=$false,
                   Position=5)]
            [String]$HTMLFile = ".\$($myinvocation.mycommand.Name.Split(".")[0])_$(Get-Date -format yyyyMMdd-hhmmsstt).html",
        [Parameter(Mandatory=$false,
                   Position=6)]
            [String]$LogFile = ".\$($myinvocation.mycommand.Name.Split(".")[0])_$(Get-Date -format yyyyMMdd-hhmmsstt).txt"
    )
...
"@
        $Error.Clear()
        if ($MaxFileSize -le $MinFileSize) {
            log 'Error: $MaxFileSize', $MaxFileSize, 'must be greater than $MinFileSize', $MinFileSize Magenta,Yellow,Magenta,Yellow $LogFile
            break
        }
    }
    ...
    Process {
        ...
        foreach ($TargetFolder in $TargetFolders) {
            ...
            foreach ($FileType in $FileTypes) {
                log "Searching for files between $MinFileSize and $MaxFileSize bytes, with '$FileType' extension, in folder '$TargetFolder'" Cyan $Logfile
                ...
                ForEach ($File in $Files) {
                    if ($File.Length -ge $MinFileSize -and $File.Length -le $MaxFileSize) {
                        $MyObjProps = @{}
                ...
                if ($FileList.Count -gt 0) { # Skip empty tables
                    log "  Found '$($FileList.Count)' files that match these criteria" Green $LogFile
                    $Params = @{
                        'As'='Table'
                        'PreContent'="<h2>'$($FileList.Count)' files with '$FileType' extension under folder '$TargetFolder' (File Length in $SizeUnit) between '$MinFileSize' and '$MaxFileSize' bytes</h2>"
                    ...
                    log "  No matching files found" Yellow $LogFile
                }
            } # File Types
        } # Search Folders
        $PreContent = "<h1>Report of files with '" + $FileTypesString.Substring(0,$FileTypesString.Length-2) + "' extension(s)"
        $PreContent += "<br> on folder(s) '" + $TargetFoldersString.Substring(0,$TargetFoldersString.Length-2) + "'</h1>"
        if (! $Tables) { $Tables = "No files found that match input criteria" }
        $Tables += "<hr> Report run time: $(Get-Date -format 'MM/dd/yyyy hh:mm:ss tt')"
        $Params = @{
            'CssStyleSheet'=$Style
        ...
        If ($Error.Count -gt 0) {
            log "Errors encountered: $Error" Yellow $Logfile
            log "Expect to see errors for file paths > 260 characters, and folders where you have no NTFS permissions" Yellow $Logfile
            log "Those files are not included in the report" Yellow $Logfile
        } 
    }

    End {
        log "Done. Report saved to file $HTMLFile" Cyan $Logfile
        & $HTMLFile
    }
}